...
const Dashboard = () => {
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      ...
      const mapped = (Array.isArray(raw) ? raw : []).map((inv) => {
        ...
        const currency = (inv.currency || 'MYR').toUpperCase()
  ...
  const HARD_RATES = {
    USD_TO_MYR: 4.07, //any value
  }

  //convert to MYR
  function convertToMYR(amount = 0, currency = 'MYR') {
    const n = Number(amount || 0)
    const curr = String(currency || 'MYR')
      .trim()
      .toUpperCase()

    if (curr === 'MYR') return n
    if (curr === 'USD') return n * HARD_RATES.USD_TO_MYR
    return n
  }

  //KPI computation with percentage ie(convert each invoices to myr before summarizing)
  const kpis = useMemo(() => {
    const totalInvoices = storedInvoices.length
    let totalPaid = 0 // in MYR
    let totalUnpaid = 0 // in MYR
    let paidCount = 0
    let unpaidCount = 0
    ...
    storedInvoices.forEach((inv) => {
      ...
      const invCurrency = inv.currency || 'MYR'
      const amtInMYR = convertToMYR(rawAmount, invCurrency)
  ...
  return (
    <div className={dashboardStyles.pageContainer}>
      ...
      <div className={dashboardStyles.kpiGrid}>
        ...
        <KpiCard
          title='Total Paid'
          value={currencyFmt(kpis.totalPaid, 'MYR')}
          hint='Received amount (MYR)'
          iconType='revenue'
          trend={12.2}
        />
        <KpiCard
          title='Total Unpaid'
          value={currencyFmt(kpis.totalUnpaid, 'MYR')}
          hint='Outstanding balance (MYR)'
          iconType='clock'
          trend={-3.1}
        />
      </div>
